...
const PER_PAGE = 4; // 4 papers per page as requested
...
function renderFunnel() {
  $("#funnelBody").innerHTML = UNIS.map(u => {
    ...
    return `<tr${dim}>
      ...
      <td class="n">${has ? fmt(c) : "—"}</td><td class="n">${has ? fmt(f.validated) : "—"}</td><td class="n" style="color:var(--accent)"><b>${fmt(t)}</b></td>
      <td><div class="stack" title="${fmt(t)} / ${fmt(f.validated)} / ${fmt(c)}">
        <i style="width:${t / c * 100}%;background:var(--ink)"></i>
        <i style="width:${Math.max(0, f.validated - t) / c * 100}%;background:var(--faint)"></i>
      </div></td></tr>`;
  }).join("");
}
