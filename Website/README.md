...
الموقع يقرأ الأبحاث من `research.final_dataset` وإحصاءات التنظيف والتحقق من `research.source_stats`.

في مشروع Azure، تتولى خطوة `UpdateStats` تحديث الإحصاءات بعد نجاح فحص الجودة.

لإنشاء جدول الإحصاءات عند الحاجة فقط، دون تغيير الأرقام الموجودة:

```bash
python setup_stats.py
```

لا يلزم تشغيل هذا السكربت بعد كل تشغيل لبايبلاين Azure.
لا تستخدموا `run_summary.json` من تشغيل محلي لتحديث إحصاءات Azure.
